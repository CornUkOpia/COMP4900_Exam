...
### Variables
- Columns are called Variables, Features or Attributes.
### Output Variables
...
- c. 1
- **d. Slope**
    - y2−y1=(w1(x+1)+w0)−(w1(x)+w0)=w1, so the correct answer is slope.
###### 3. 
- ![](Images/PRACTICEEXAM1.png)
- ![](Images/PRACTICEEXAM1.2.png)
...
- Gradient descent is an iterative optimisation algorithm to find the minimum of a function.
    - To find a local minimum of a function using gradient descent, one takes steps proportional to the negative of the graident of the function at the current point.
...
- Overfitting corresponds to a machine learning model with high variance (and usually low bias).
#### Generalisation:
...
#### Bias
- Bias is the error resulting from the difference between the expected value(s) of a model and the actual (or “correct”) value(s) for which we want to predict over multiple iterations.
#### Variance
- Variance is defined as the error resulting from the variability between different data predictions in a model.
#### Bias-Variance Trade-off 
- The idea that as error due to bias decreases error due to variance increases, creating a trade-off which should be minimized in model validation, and other circumstances.
### Uses of data
...
- Model/hypothesis A: Linear regression with basic features 
    - ErrA = 1/5 ∑j=1:5 Err^A_I (Average error of model A across all folds.)
- Model/hypothesis B: Linear regression with quadratic expansions of features
    - ErrB = 1/5 ∑j=1:5 Err^B_I (Average error of model B across all folds.)
...
## Logistic Regression
-  To reduce statistical variance, use L2-Regularisation
...
- LR models the probabilities of an observation belonging to each of the classes via linear functions. It is generally considered safer and more robust than discriminant analysis approaches, as it is relying on fewer assumptions. 
##### QUIZ QUESTION
...
###### 2. A classifier trained on less training data is less likely to overfit.
- True
- **False**
    - Increasing the amount of training data will lead to lower variance models that are less likely to overfit.
...
- The decision boundary is the set of points for which the linear model predicts zero. a = 0.
    ...
    - pHat(y = 1 | x) = 𝜕(w^T * X) = 1 / (1 + e^-W^T*X)
    - Logistic regression has a decision boundary that is linear in X.
##### QUIZ QUESTIONS
###### 1. Suppose that we have the following logistic prediction model: P(y=1|x)= 1 / 1+e^(0.1 − 0.6 X1 + 1.2 X2) Compute the predicted log-odds ratio for the point x=[4,2].
...
### Gradient Descent
- 𝜕Err(W)/𝜕w = - ∑i=1:n Xi (Yi - 𝜕(w^T * Xi))
    - Yi = True output.
    - 𝜕(w^T * Xi) = Predicted output.
- Update rule: Wk+1 = Wk + aK * ∑i=1:n Xi (Yi - 𝜕(w^T * Xi))
- Move in a direction that makes our prediction better.
- The algorithm:
- Given an initial weight vector w0,
- Do for k=0,1,2,...
    - Calculate T = 𝜕Err(W)/𝜕w = - ∑i=1:n Xi (Yi - 𝜕(w^T * Xi))
...
- After estimating w using gradient descent, calculate probabilities for a given new feature vector Xnew as follows:
    - p(yNew = 1 | xNew) = 𝜕(w^T * xNew)
    - p(yNew = 0 | xNew) = 1 - 𝜕(w^T * xNew)
...
- It assumes that the joint density of all features, conditional on the target's class, is a multivariate Gaussian. 
...
###### Under what circumstances would the solutions of LDA and QDA be the same?
- a. All features are independent. 
- b. The Naive Bayes assumption holds.
- c. The number of features is greater than the number of parameters.
- **d. None of the above.**
    - LDA and QDA would only be equivalent if the features had the same variance (or co-variance) for the different classes. The correct answer is: None of the above.
...
- QDA relaxes the common covariance assumption of LDA by estimating a separate covariance matrix for each class. This gives the model more flexibility, but in the case of many features can lead to a dramatic increase in the number of parameters in the model.
- ![](Images/LDAvsQDA.png)
##### QUIZ QUESTION
...
## Naive Bayes
- To reduce statistical variance, use Laplace smoothing.
- P(S) shows the probability (P) of something (S);
- P(S | T) shows the probability (P) of something (S) given that we know a thing (T);
- A Naive Bayes classifier performs better compare to other models like logistic regression and you need less training data.
...
###### 2. An employee of a fledgling sports analysis company is attempting to use Naive Bayes to predict whether a hockey team will score more than 5 goals in a game. The three features she is investigating are: topten (whether the team is in the top-10 of the NHL standings), injuries (whether the team has more than 3 players on injured reserve), newcoach (whether the team switched their coach within the last 50 games).
###### The training points she has collected so far are:
    y=1, [topten = 1; injuries = 1; newcoach = 1]
    y=0, [topten = 0; injuries = 0; newcoach = 0]
    y=0, [topten = 0; injuries = 1; newcoach = 0]
    y=0, [topten = 0; injuries = 0; newcoach = 0]
    y=0, [topten = 1; injuries = 1; newcoach = 0]
    y=1, [topten = 1; injuries = 1; newcoach = 0]
###### Part 1. (7 points) What are the parameters of the Naive Bayes model trained on this data, without Laplace smoothing? 
- θ1=1/3=0.33
- θtopten,1=1
- θtopten,0=1/4=0.25
- θinjuries,1=1
...
- θnewcoach,1=1/2=0.5
- θnewcoach,0=0
###### Part 2. (4 points) Consider a team with the features [topten = 0, injuries = 0, newcoach = 0]. What probability would the Naive Bayes model give for this team scoring more than 5 goals? 
###### Part 3. (7 points) Suppose we used Laplace smoothing to train the Bernoulli Naive Bayes model. Would the predicted class for the new data point in Part 2 change?
### Training
...
- Replace the maximum likelihood estimate
    - 𝛳j,1 = P(Xj|y=1) = (number of instances with Xj = 1 and Y = 1) / (number of examples with Y = 1) with 𝛳j,1 = P(Xj|y=1) = ((number of instance with Xj = 1 and Y=1) + 1)/((Number of examples with y=1)+2)
    ...
    - You should laplace smooth both P(Xj |y=1) = 𝛳j,1 and P(Xj | y=0) = 𝛳j,0
##### QUIZ QUESTION
...
-  Lasso regression not only helps in reducing over-fitting but it can help us in feature selection. Just like Ridge regression the regularization parameter (lambda) can be controlled
...
- Discrete--, discrete-output case:
    ...
    - Example: For Boolean functions, the truth table row → path to leaf.
- Continuous--, continuous-output case:
    - Can approximate any function arbitrarily closely
### Classification and regression
- Each path from root to a leaf denotes a region 𝑅𝑟
of - space.
- Assume you built the tree:
    - Let (X1^𝑟, Y1^𝑟), ... , (Xk^𝑟, Yk^𝑟) be the training examples that fall into 𝑅𝑟.
    - Classification tree:
        - Discrete output (e.g. class orange vs class lemon)
        - leaf value    Y^𝑟, i.e. class label of region r, typically set to the most common value in  (Y1^𝑟, Yk^𝑟)
    - Regression tree
        - Continuous output
        - leaf value Y^r typically set to the mean value in (Y1^𝑟,..., Yk^𝑟).
...
- Specific Conditional Entropy
    ...
    - What is the entropy of cloudiness Y, given that it is raining:
        - H(Y|X = raining) = - ∑ P(Y|raining) log2 P(Y|raining)
...
###### 2. Suppose we have the following training points:
    x=[0,0],y=1
    x=[0,1],y=0
    x=[1,0],y=0
    x=[1,1],y=1
###### Part 1. (2 points) Does there exist a linear perceptron that can correctly classify every point in this training set? 
- The data is not linearly separable (it is the Boolean negation of the XOR function), so there does not exist a linear perceptron that can correctly classify every point in this training set.
###### Part 2. (2 points) Could a decision tree with binary tests achieve 100% accuracy on the four training points? If so, what is the minimum number of tests required? 
- A depth-2 decision tree with 3 binary tests could perfectly classify this data, and 3 is the minimum number of tests required. A single test can only split the data along the vertical or horizontal axis, and this split does not achieve 100% accuracy (with there being two points of each class on each side of the test). Two more tests are required to split the data into the necessary four axis-orthogonal quadrants.
###### Part 3. What training accuracy would a 3-nearest-neighbor model achieve on this dataset?  
- A 3-nearest neighbor model would achieve 0% accuracy; the nearest neighbor to every point is itself but the other two nearest neighbors are always from the opposite class.
###### 3. Suppose you are learning a decision tree for email spam classification. Your current sample of the training data has the following distribution of labels: [43+, 30-], i.e., the training sample has 43 examples that are spam and 30 that are not spam. Now, you are choosing between two candidate tests. Test 1 (T1) tests whether the number of words in the email is greater than 30 and would result in the following splits. num-words > 30 : [5+, 18-], num-words <= 30: [38+, 12-]. Test 2 (T2) tests whether the email contains an external URL link and would result in the following splits. has-link: [28+, 8-], not-has-link: [15+, 22-] Which test should you use to split the data? I.e., which test provides a higher information gain?
- a. T2 provides a higher information gain.
- b. They provide the same information gain.
- **c. T1 provides a higher information gain.**
- d. It depends on whether a base-2 or base-10 logarithm is used.
    - The conditional entropy of Test 1 is 0.782, while the conditional entropy of Test 2 is 0.87. Thus,  Test 1 provides more information. 
...
- Idea: Project data into a lower-dimensional sub-space, Rm →Rd, where d < m. 
- Principal component analysis (PCA), which performs a linear mapping of the data to a lower-dimensional space in such a way that the variance of the data in the low-dimensional representation is maximized.
- Consider a linear mapping, xi → P^T * xi
    - xi is the original 𝑚 × 1 feature vector for the i-th sample.
    - P is the compression matrix with dimension 𝑚 × 𝑑.
        - P^Txi is d × 1
    - Assume there is a decompression matrix U^m×d
- Solution can be obtained by eigen-decomposition of X^T * X.
    ...
    - P is m×d matrix corresponding to the d selected eigenvectors.
    - Note: eigen vectors are orthogonal ➔ The columns of P are orthogonal!
...
- Mutual information between variable Xj and target Y: I(Xj,Y) = ∑Xj ∑Y P(Xj = xj, Y=y) log( (P(Xj = xj, Y=y))/P(Xj = xj) P(Y=y))
...
-  To reduce statistical variance, increase the value of k.
- Given: Training Data X, distance d on X
- Learning: Nothing to do, just store the data
...
###### 3. The training error of a one-nearest neighbor classifier is always 0.
- **True**
- False
    - The nearest neighbor of a training point is always itself, so training points will always be correctly classified.
### Limitations
...
- Predict: 
    ...
    - Predict: 
        - For regression: ynew = ∑i=1:n wiyi / ∑i=1:n wi
...
- Weak learners are models that can be used as building blocks for designing more complex models by combining several of them. Most of the time, these basics models perform not so well by themselves either because they have a high bias or because they have too much variance to be robust.
### Bagging
...
- How to make predictions using a committee of classifiers
    - Weight the binary prediction of each classifier by the quality of that classifier:
        - yHat(x) = sign (∑ t=1:B At * yHatT(x))
            - yHatT(x) ∈ {-1,+1}, yHatT(x) is prediction uusing the t classifier
...
- Data moves in only one direction forward from input until it reaches the output. Along the way, the sum of the products of the inputs and weights are calculated. The end result is passed to the outputs for processing.
...
- 𝐡(𝑖): hidden vector corresponding to the ith hidden layer.
- 𝐖(𝑖): weight matrix corresponding to the ith hidden layer.
- 𝐛(𝑖): bias vector corresponding to the ith hidden layer.
...
- Initialize all weights and 𝑏s to small random numbers.  (Initialisation)
...
##### QUIZ QUESTION
###### 1. What is true about Stochastic Gradient Descent?
...
- Dropout means to drop out units that are covered up and noticeable in a neural network. Dropout is a staggeringly in vogue method to overcome overfitting in neural networks.
- Preceding Dropout, a significant research area was in regularization. The introduction of regularization methods in neural networks, for example, L1 and L2 weight penalties, began from the mid-2000s. Notwithstanding, these regularizations didn’t totally tackle the overfitting issue.
...
- P(yHat = k) = softmax(z,k) = (e^Zk)/(∑ j=1:K e^Zj)
    ...
    - ∑ j=1:K e^Zj: Sum over all entries of z.
...
- Recurrent Neural Networks (RNNs) add an interesting twist to basic neural networks.
- Recurrent Neural Network remembers the past and it’s decisions are influenced by what it has learnt from the past. Note: Basic feed forward networks “remember” things too, but they remember things they learnt during training. 
...
- But it also gets some information 𝒉^<1> from the previous network. 𝒉^<1> is called hidden state at time 1.
...
- Each - sample 𝑿 is a sequence. Output labels 𝒚 are also a sequence.
- 𝑇𝑥 and 𝑇𝑦 are the length of the - and output.
### Why RNN?
- - length would be too long.
    - Assume each - is a word represented by a one-hot vector of the length 10,000. Then the - length would be 𝑇𝑥 × 10,000.
...
- Intuition: Imagine multiplying a scalar number w by itself many times. w^k for k → ∞ will either explode (if w > 1) or vanish (if w < 1).
- Similar behavior occurs if 𝑾ℎℎ is a matrix.
- If gradient explode → Solution: Clip the gradient.
- If gradient vanish → Solution: Change the architecture so there are some non-multiplicative interactions → LSTM
...
- SVM is a supervised learning algorithm which can be used for classification and regression problems as support vector classification (SVC) and support vector regression (SVR). It is used for smaller dataset as it takes too long to process. 
- SVM is based on the idea of finding a hyperplane that best separates the features into different domains.
- Basically, SVM is composed of the idea of coming up with an Optimal hyperplane which will clearly classify the different classes
...
- c. M=1/2
- d. M=1/2 √1/2
    - By drawing out the dataset, one can observe that the line defined by w=[-1,1] and b=1 (i.e., the line that goes through [1,0] and [0,-1]) correctly splits the data with the negative point (x3=[1, -1]) on one side and the three other positive points on the other. Moreover, the line w=[-1, 1] with b=1 is equidistant from the negative point x3=[1, -1] and the two positive points, x1=[-1, -1] and x4=[1, 1], with the line being a distance of √1/2 from each of these three points, which thus must be the support vectors. This gives us the size of the margin as M=2√1/2
...
###### 6. 
- ![](Images/SVMPRACTICEQUESTION.png)
- ![](Images/SVMPRACTICEANSWER.png)
...
- What is a Hyperplane?  The hyperplane is a function which is used to differentiate between features.
    -  In 2-D, the function used to classify between features is a line whereas, the function used to classify the features in a 3-D is called as a plane similarly the function which classifies the point in higher dimension is called as a hyperplane
...
- The decision boundary looks like a line because 𝒙 ∈ ℝ2, but think about it as a m-dimensional hyperplane.
- ![](Images/SEPARATINGHYPERPLANES1.png)
- Recall that a hyperplane is described by points 𝒙 ∈ ℝ𝑚 such that 𝑓 𝒙 = 𝒘^𝑇 * 𝒙 + 𝑏 = 0.
...
- Recall that the decision hyperplane f(x) is orthogonal (perpendicular) to 𝐰.
- The vector wnormal = 𝐰/||𝐰||2 is a unit vector pointing in the same direction as 𝐰.
- Any point on the plane (e.g. 𝑥1) satisfies plane equation (e.g. 𝑓(𝑥1) = 0)
...
- Enforcing a margin of C:
    - Yi * ((w^T * Xi + b)/(||𝐰||2)) >= C       i=1,...,n
- This is not very convenient for optimization.
    - 𝐰 appears nonlinearly in the constraints.
    - Problem is under-constrained. If (w, b, C) is optimal, so is (βw, βb , C), for any β>0
    - Add a constraint: 𝐶 = 1/(||𝐰||2) 
- Instead try the equivalent optimization problem: 
- Observe: if the margin constraint is not tight for 𝒙𝑖, we could remove it from the training set and the optimal 𝐰 would be the same.
...
- Using Lagrangian, it can be shown that w* (the optimal w) is w* = ∑i=1:n ai yi xi
...
- If 0 < 𝜁𝑖 < 1 , 𝑥𝑖 will still be correctly classified (functional marginal positive but less than 1).
- If If 𝜁𝑖 > 1, 𝑥𝑖 will be misclassified (negative functional margin).
...
- Here xi is replaced by ɸ(xi), where ɸ is called a feature mapping.
- Example:
    - Original 2-D feature vector x = [𝑥1, 𝑥2]^⊤
    - Replace x with expanded 3-D vector
        - 𝛷(x) = [𝑥1, 𝑥2, 𝑥1^2 + x2^2]^T
    - Apply SVM
##### 1.
- ![](Images/DECISIONBOUNDARY1.png)
##### Answer
- Part 1. The data is not linearly separable (it is the Boolean negation of the XOR function), so there does not exist a linear perceptron that can correctly classify every point in this training set.
- Part 2. A depth-2 decision tree with 3 binary tests could perfectly classify this data, and 3 is the minimum number of tests required. A single test can only split the data along the vertical or horizontal axis, and this split does not achieve 100% accuracy (with there being two points of each class on each side of the test). Two more tests are required to split the data into the necessary four axis-orthogonal quadrants.
- Part 3. A 3-nearest neighbor model would achieve 0% accuracy; the nearest neighbor to every point is itself but the other two nearest neighbors are always from the opposite class.
...
- Conversely, by choosing feature mapping ɸ, we implicitly choose a kernel function.
...
- Is this a kernel, K(x, z) =? ɸ(x). ɸ(z)
    - K(x, z) = (∑i=1:m xi zi) (∑j=1:m xj zj)
    - = ∑i,j𝜖{1…m} xi zi xj zj
    - = ∑i,j 𝜖{1...m} ( xi xj) ( zi zj)
    - = ɸ(x). ɸ(z)
...
- Important: Computing ɸ takes O(m2) but computing K only takes O(m).
...
#### Kernel Trick
- The kernel is a way of computing the dot product of two vectors x and y in some (very high dimensional) feature space, which is why kernel functions are sometimes called “generalized dot product. Applying kernel trick means just to the replace dot product of two vectors by the kernel function.
...
- Replace the inner product with Kernel K; in this way you
    ...
    - Do not have to ever compute the high dimensional feature mapping ɸ;
        - Think of ɸ as implicit, not explicit.
...
- Using Taylor expansion of 𝑒^𝐱, It can be shown that
...
- K-means Objective:
    - Find cluster centers {𝐦𝑘}^K 𝑘=1 and assignments {𝐫^(𝑛)}^N 𝑛=1
        ...
        - Responsibilities: Cluster assignment for sample 𝑛: 𝐫^(𝑛) ∈ ℝ𝐾 1-of-K encoding (not observed)
        - Mathematically: minimize the sum of squared distances of data points 𝐱(𝑛) to their assigned cluster centers :
...
- E.g. say sample 𝒙^(𝒏) is assigned to cluster k = 3, then r^n = [0,0,1,0]
#### How to Optimize?:
- ![](Images/OPTIMIZEKMEANS.png)
- If we fix the centers {𝒎𝑘} then we can easily find the optimal assignments {𝐫^(𝑛)} for each sample 𝑛.
- Assign each point to the cluster with the nearest center
    - Rk^n = {1 if k = argminJ ||x^(n) - Mj||^2,     0 otherwise}
- E.g. 𝐱^(𝑛) is assigned to cluster 𝑘Hat, 
    - r^(n) = [0,0,...,1,...,0]^T
- Likewise, if we fix the assignments {𝐫^(𝑛)} then can easily find optimal centers {M𝑘}
...
- The intuition behind the algorithm is actually pretty straight forward. To begin, we choose a value for k (the number of clusters) and randomly choose an initial centroid (centre coordinates) for each cluster. 
- High level overview of algorithm:
    ...
    - The algorithm iteratively alternates between two steps:
        - Assignment step: Assign each data point to the closest cluster. Optimize 𝐽 w.r.t. {𝒓}. Each data point 𝐱^(𝑛) assigned to nearest center,
...
- Whenever an assignment is changed, the sum of squared distances 𝐽 of data points from their assigned cluster centers is reduced.
...
### Local Minima
- The objective 𝐽 is non-convex (so it is not guaranteed to converge to the global minimum).
- There is nothing to prevent k-means getting stuck at local minima.
- We could try many random starting points. Pick the run that provides smaller 𝐽.
...
- Elbow method:
    - As K increases, check the cost function 𝑱.
...
- Repeat until convergence (measured by how much 𝐽 changes):
...
- Under Gaussian assumption, a datapoint 𝐱 is generated as follows:
    - Given a cluster 𝑧 ∈ {1, … ,𝐾}, probability of generating 𝐱 is 𝒩(𝐱|𝝁𝒛,𝚺) , where 𝑝(𝑧 = 𝑘) = 𝜋𝑘.
- Task: Find model parameters 𝜇 K=2, two gaussians 𝑘 and 𝜋𝑘 using maximum likelihood. 
- This defines joint distribution 𝑝(𝑧,𝐱) = 𝑝(𝑧) 𝑝(𝑥|𝑧) with parameters {𝜋𝑘, 𝝁𝑘}K    𝑘=1
- The marginal of 𝐱 is given by 𝑝(x) = Σ𝑧𝑝(𝑧, 𝐱)
- 𝑝(𝑧 = 𝑘|𝐱) can be computed using Bayes rule
- 𝑟𝑘 tells us the probability 𝐱 came from the 𝑘𝑡ℎ cluster (like a soft assignment to clusters) 
...
- How should we choose the parameters {𝜋𝑘, 𝝁𝑘}K  𝑘=1?
- Maximum likelihood principle: choose parameters to maximize likelihood of observed data
- We don't observe the cluster assignments 𝑧, we only see the data 𝐱.
- Maximum likelihood: Given data 𝒟 = {𝐱^(n)} N      n=1, choose parameters to maximize:
- ![](Images/LEARNMODELPARAMETERSUSINGMAXLIKELIHOOD.png)
### Gaussian Mixture Model (GMM)
- This distribution is an example of a Gaussian Mixture Model (GMM), and 𝜋𝑘 are known as the mixing coefficients.
...
- How would you optimize this w.r.t. parameters {𝜋𝑘, 𝝁𝒌}?
...
- By maximizing log 𝑝(𝒟complete), we would get this:
...
two steps:
- E-step: Compute the posterior probabilities 𝑟𝑘(𝑛) = 𝑝(𝑧(𝑛) = 𝑘|𝐱(𝑛)) given our current model - i.e. how much do we think a cluster is responsible for generating a datapoint.
- M-step: Use the equations on the previous slide to update the parameters, assuming 𝑟𝑘(𝑛) are held fixed -- change the parameters of each Gaussian to maximize the probability that it would generate the data it is currently responsible for.
### EM Algorithm for GMM
- Initialize the means 𝜇Hat𝑘 and mixing coefficients 𝜋Hat𝑘
- Iterate until convergence:
    - E-step: Evaluate the responsibilities 𝑟𝑘(𝑛) given current parameters
...
- The EM Algorithm:
    1. E-step: Compute the posterior probability over 𝑧 (responsibilities) given our current model
